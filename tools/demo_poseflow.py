...
def demo(sess, net, image_name,xminarr,yminarr,xmaxarr,ymaxarr,results,score_file,index_file,num_boxes):
 ...
 # im_file = os.path.join(cfg.DATA_DIR, 'coco-test-dev', image_name)
    im_file = os.path.join('/SSD3/jeff/data/bonn_small', 'images', image_name)
...
if __name__ == '__main__':
    ...
    im_names = [line.rstrip('\n') .rstrip('\r') for line in open("/home/jeff/poseflow/bonn-small/test_list.txt")]
    ...
    xmaxarr=[]
    ymaxarr=[]
    if (os.path.exists("./"+inference_set+"/") == False):
        os.mkdir("./"+inference_set+"/")
    results = open(inference_set+"/test-dev_images.txt", 'w')
    score_file = open(inference_set+"/score-proposals.txt",'w')
    index_file = open(inference_set+"/index.txt",'w')
    num_boxes = 0
    for im_name in im_names:
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print('Demo for data/{}/{}'.format(inference_set,im_name))
        num_boxes=demo(sess, net, im_name,xminarr,yminarr,xmaxarr,ymaxarr,results,score_file,index_file,num_boxes)
    with h5py.File(inference_set+'/test-dev.h5', 'w') as hf:
