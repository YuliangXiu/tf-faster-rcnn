...
NETS = {'vgg16': ('vgg16_faster_rcnn_iter_70000.ckpt',),'res101': ('res101_faster_rcnn_iter_110000.ckpt',),'res152':('res152_faster_rcnn_iter_490000.ckpt',)}
...
def demo(sess, net, image_name,xminarr,yminarr,xmaxarr,ymaxarr,results,score_file,index_file,num_boxes):
    """Detect object classes in an image using pre-computed object proposals."""

    # Load the demo image
    #im_file = os.path.join(cfg.DATA_DIR, 'coco-minival_images', image_name)
    im_file = os.path.join(cfg.DATA_DIR, 'aic-val', image_name)
    ...
    # Visualize detections for each class		r
    CONF_THRESH = 0.15
    NMS_THRESH = 0.7
    # Visualize people
    cls_ind = 1 
...
if __name__ == '__main__':
    ...
    tfmodel = os.path.join('../output', demonet, DATASETS[dataset][0], 'backup',
                              NETS[demonet][0])
    #print(tfmodel)	
    #import time
    #time.sleep(10)
    ...
    #im_names = [line.rstrip('\n') .rstrip('\r') for line in open("../data/aic-minival5k.txt")]
    im_names = [line.rstrip('\n') .rstrip('\r') for line in open("../data/aic-val.txt")]
    ...
    xmaxarr=[]
    ymaxarr=[]
    #results = open("aic-val-minival5k/test-dev_images.txt", 'w')
    #score_file = open("aic-val-minival5k/score-proposals.txt",'w')
    #index_file = open("aic-val-minival5k/index.txt",'w')
    
    results = open("aic-val-new/test-dev_images.txt", 'w')
    score_file = open("aic-val-new/score-proposals.txt",'w')
    index_file = open("aic-val-new/index.txt",'w')
    
    num_boxes = 0
    for im_name in tqdm(im_names):
        #print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        #print('Demo for data/coco-minival_image/{}'.format(im_name))
        num_boxes=demo(sess, net, im_name,xminarr,yminarr,xmaxarr,ymaxarr,results,score_file,index_file,num_boxes)
    with h5py.File('aic-val-new/test-dev.h5', 'w') as hf:
    #with h5py.File('aic-val-minival5k/test-dev.h5', 'w') as hf:
