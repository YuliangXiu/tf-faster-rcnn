...
def demo(sess, net, image_name,xminarr,yminarr,xmaxarr,ymaxarr,results,score_file,index_file,num_boxes):
    ...
    im_file = os.path.join(cfg.DATA_DIR, 'aic-test-a', image_name)
...
if __name__ == '__main__':
    ...
    im_names = [line.rstrip('\n') .rstrip('\r') for line in open("../data/aic-test-a.txt")]
    ...
    xmaxarr=[]
    ymaxarr=[]
    results = open("aic-test-a-2/test-dev_images.txt", 'w')
    score_file = open("aic-test-a-2/score-proposals.txt",'w')
    index_file = open("aic-test-a-2/index.txt",'w')
    num_boxes = 0
    ...
    with h5py.File('aic-test-a-2/test-dev.h5', 'w') as hf:
