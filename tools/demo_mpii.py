...
def demo(sess, net, image_name,xminarr,yminarr,xmaxarr,ymaxarr,results,score_file,index_file,num_boxes):
    ...
    im_file = os.path.join('../data/MPII/images',image_name)
...
if __name__ == '__main__':
    ...
    for i in xrange(len(tags)):
        tag = tags[i]
        os.system("mkdir -p MPII/{}".format(tag))
        im_names = [line.rstrip('\n') .rstrip('\r') for line in open("../data/MPII/{}.txt".format(tag))]
        ...
        xmaxarr=[]
        ymaxarr=[]
        results = open("MPII/{}/test-dev_images.txt".format(tag), 'w')
        score_file = open("MPII/{}/score-proposals.txt".format(tag),'w')
        index_file = open("MPII/{}/index.txt".format(tag),'w')
        num_boxes = 0
        for im_name in im_names:
            print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
            print('Demo for data/MPII/{}/{}'.format(tag, im_name))
            num_boxes=demo(sess, net, im_name,xminarr,yminarr,xmaxarr,ymaxarr,results,score_file,index_file,num_boxes)
        with h5py.File("MPII/{}/test-dev.h5".format(tag), 'w') as hf:
