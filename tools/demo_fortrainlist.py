...
def demo(sess, net, image_name,xminarr,yminarr,xmaxarr,ymaxarr,results,score_file,index_file,num_boxes):
 ...
 # im_file = os.path.join(cfg.DATA_DIR, 'coco-test-dev', image_name)
    im_file = os.path.join(cfg.DATA_DIR, 'train2014', image_name)
...
if __name__ == '__main__':
    ...
    im_names = [line.rstrip('\n') .rstrip('\r') for line in open("../data/train_list.txt")]
    ...
    xmaxarr=[]
    ymaxarr=[]
    results = open("train_list/test-dev_images.txt", 'w')
    score_file = open("train_list/score-proposals.txt",'w')
    index_file = open("train_list/index.txt",'w')
    num_boxes = 0
    ...
    with h5py.File('train_list/test-dev.h5', 'w') as hf:
