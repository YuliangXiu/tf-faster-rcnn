...
def display_pose(intputpath, outputpath, imgpath):
    vidname, imgname = imgpath.split('/')
    img = Image.open(os.path.join(intputpath,imgname))
    width, height = img.size
    fig = plt.figure(figsize=(width/10,height/10),dpi=10)
    plt.imshow(img)
    for pid in range(len(rmpe_results[imgpath])):
        pose = np.array(rmpe_results[imgpath][pid]['keypoints']).reshape(-1,3)[:,:2]
        if pose.shape[0] == 16:
            ...
            for idx in range(len(colors_skeleton)):
                plt.plot(np.clip(pose[pairs[idx],0],0,width),np.clip(pose[pairs[idx],1],0,height),color=colors_skeleton[idx],linewidth=40)
            ...
            for idx in range(len(colors_skeleton)):
                plt.plot(np.clip(pose[pairs[idx],0],0,width),np.clip(pose[pairs[idx],1],0,height),
                         color=colors_skeleton[idx],linewidth=40)
...
if __name__ == '__main__':
    ...
    jsonpath = os.path.join(args.outputpath,"/POSE/great-pose-results.json")
    
    with open(jsonpath) as f:
        rmpe_results = json.load(f)
    for imgpath in tqdm(rmpe_results.keys()):
        display_pose(inputpath, outputpath, imgpath)    
